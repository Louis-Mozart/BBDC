...
Although we were unable to reproduce the same score achieved on the BBDC dashboard, we adjusted our model to produce more consistent and reproducible results. The hyperparameters used for our model are printed during preprocessing, training, and postprocessing. Please refer to the terminal output when the model is running.
